...
const HelloWorldSceneAR = () => {
  ...
  const fetchSensorData = useCallback(async () => {
    if (isLoading || pauseFetch) return; // No actualizar si pausado
    setIsLoading(true);
    setError(null);

    try {
      const response = await fetch(
        "http://192.168.1.14:8080/api/sensor-data",
        {
          method: "GET",
      ...
      // Solo actualizar con datos aleatorios si NO está pausado
      if (!pauseFetch) {
  ...
  }, [isLoading, pauseFetch]);

  useEffect(() => {
    fetchSensorData(); // llamada inmediata
  ...
  const updateText =
    (isLoading ? "Actualizando..." : error ? "X " + error : "Conectado") +
    " | " +
    lastUpdate;

  // Función para seleccionar un valor y congelar temporalmente
  const selectTemperature = (value: string) => {
    setTemperature(value);
    setPauseFetch(true);
    setIsEditingTemp(false);
    setTimeout(() => setPauseFetch(false), 10000); // reanudar fetch después de 10s
  };

  const selectHumidity = (value: string) => {
    setHumidity(value);
    setPauseFetch(true);
      ...
      {isEditingTemp && (
        <>
          <ViroText
            ...
            onClickState={(s) => {
              if (s === 1) selectTemperature("30.0°C");
            }}
          />
          <ViroText
            text="32.5°C"
            ...
            style={styles.optionButton}
            onClickState={(s) => {
              if (s === 1) selectTemperature("32.5°C");
            }}
          />
          <ViroText
            text="Cancelar"
